...
$totalRows_Rs_GetvStoreCategories = mysql_num_rows($Rs_GetvStoreCategories);
 ...
 if ($totalRows_Rs_GetvStoreCategories > 0) { // Show if recordset not empty ?>
  <ul>
    <?php do { ?> 
      <!-- Start of Category Item -->
      <li class="MainMenu">
        <h3> <?php echo $row_Rs_GetvStoreCategories['VSItemMainCategoryName']; ?></h3>
        <ul  class="SubMenus">
          <?php
		...
		do { ?>
            <li><a href="" catid="<?php echo $row_Rs_GetvStoreCategories['VSItemMainCATID'];  ?>" subcatid="<?php echo $row_RsGetSubCategories['VSItemSubCATID']; ?>"  class="VSvirtualstorelink"><?php echo $row_RsGetSubCategories['VSItemSubCategoryName']; ?></a></li>
