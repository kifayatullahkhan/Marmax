...
$colname_Rs_CheckItemsCountInSubCAT = "-1";
if (isset($_GET['subcatid'])) {
  $colname_Rs_CheckItemsCountInSubCAT = $_GET['subcatid'];
}
mysql_select_db($database_Conn, $Conn);
$query_Rs_CheckItemsCountInSubCAT = sprintf("SELECT * FROM vs_users_virtual_stores_inventory WHERE VSItemSubCatID = %s", GetSQLValueString($colname_Rs_CheckItemsCountInSubCAT, "int"));
$Rs_CheckItemsCountInSubCAT = mysql_query($query_Rs_CheckItemsCountInSubCAT, $Conn) or die(mysql_error());
$row_Rs_CheckItemsCountInSubCAT = mysql_fetch_assoc($Rs_CheckItemsCountInSubCAT);
$totalRows_Rs_CheckItemsCountInSubCAT = mysql_num_rows($Rs_CheckItemsCountInSubCAT);

$maxRows_Rs_Show_CategoryItems = 6;
...
if (isset($_GET['subcatid'])) {
  $colname_Rs_Show_CategoryItems = $_GET['subcatid'];
}
$SearchBoxText_Rs_Show_CategoryItems = "";
if (isset($_GET['SearchBoxText'])) {
  $SearchBoxText_Rs_Show_CategoryItems = $_GET['SearchBoxText'];
}
mysql_select_db($database_Conn, $Conn);
if($colname_Rs_Show_CategoryItems=="0" || $totalRows_Rs_CheckItemsCountInSubCAT==0){
$query_Rs_Show_CategoryItems = sprintf("SELECT * FROM vs_users_virtual_stores_inventory");
}else{
$query_Rs_Show_CategoryItems = sprintf("SELECT * FROM vs_users_virtual_stores_inventory WHERE VSItemSubCATID = %s", GetSQLValueString($colname_Rs_Show_CategoryItems, "int"));	
}
// Start of the Search Quiery
if(isset($_GET['SearchBoxText'])&& $_GET['SearchBoxText']!=="jQueryScript") {
	$query_Rs_Show_CategoryItems = sprintf("SELECT * FROM vs_users_virtual_stores_inventory WHERE VSItemSubCATID = %s OR vs_users_virtual_stores_inventory.VSInvItemName LIKE %s", GetSQLValueString($colname_Rs_Show_CategoryItems, "int"),GetSQLValueString("%".$SearchBoxText_Rs_Show_CategoryItems."%", "text"));
}// End of Search Enabled Query
...
if($colname_Rs_Show_CategoryItems=="0"){
	ZorkifMessageBox("You can Choose Category to Narrow your search results","Choose Category");
}else
  if($totalRows_Rs_CheckItemsCountInSubCAT==0){
	ZorkifMessageBox("Could not Find any Item in the current selection <br/> Following are other items you might be intrested","No Item(s) Found");
  }
?> 
  <?php
  if($totalRows_Rs_Show_CategoryItems>0){
   do { ?>
	...
	<div class="ItemDisplayBorder">
            <div class="ItemImage">
            <a href="show_item_details_en.php?InvItemID=<?php echo $row_Rs_Show_CategoryItems['VSInvItemCode']; ?>">
        ...
        <div class="ItemDescription">
            <div class="Title"><?php echo $row_Rs_Show_CategoryItems['VSInvItemName']; ?></div>	
               <?php echo $row_Rs_Show_CategoryItems['VSInvItemShortDescription']; ?> 
            <div class="AddToCartButton">
              <a href="" Quantity="1" ItemCartType="VirtualStore" InvItemID="<?php echo $row_Rs_Show_CategoryItems['VSInvItemCode']; ?>"></a>
            </div>
            <div class="PriceTag">SAR <?php echo $row_Rs_Show_CategoryItems['VSInvItemSalesPrice']; ?></div> 
...
?>
<?php 
if(file_exists("php_code_files/shopping_cart/add_item_cart_jquery_inc_en.php")) {
require_once('php_code_files/shopping_cart/add_item_cart_jquery_inc_en.php'); 
}elseif(file_exists("../shopping_cart/add_item_cart_jquery_inc_en.php")) {
require_once('../shopping_cart/add_item_cart_jquery_inc_en.php'); 
}


mysql_free_result($Rs_CheckItemsCountInSubCAT);
?>
